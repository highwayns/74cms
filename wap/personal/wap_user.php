...
elseif ($act == 'favorites')
{
	...
	$ordersql=" order by favoritesa_ddtime desc ";
	...
	$smarty->assign('favorites',get_favorites($start, $perpage,$joinsql.$wheresql.$ordersql));
...
elseif($act == "resume_jobs_save")
{
	...
	if(!updatetable(table('resume'),$setsqlarr," id=$_POST[pid] "))exit("err");
...
elseif($act == "resume_work_save")
{
	...
	$resume_work=get_resume_work($_SESSION['uid'],$_POST['pid']);
...
elseif($act == "resume_education_save")
{
	...
	$resume_basic=get_resume_basic(intval($_SESSION['uid']),intval($_POST['pid']));
	if (empty($resume_basic)) exit('请先填写简历基本信息');
	$resume_education=get_resume_education($_SESSION['uid'],$_POST['pid']);
...
elseif($act == "resume_train_save")
{
	...
	$resume_basic=get_resume_basic(intval($_SESSION['uid']),intval($_POST['pid']));
	if (empty($resume_basic)) exit('请先填写简历基本信息');
	$resume_training=get_resume_training($_SESSION['uid'],$_POST['pid']);

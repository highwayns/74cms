...
elseif ($act == 'index')
{
	...
	$sql="select a.*,r.title,r.education_cn,r.birthdate,r.experience_cn,r.residence_cn from ".table("company_favorites")." as a left join ".table("resume")." as r on a.resume_id=r.id  $wheresql order by a.favoritesa_ddtime desc ".$limit;
	$row=$db->getall($sql);
...
elseif($act=="ajax_collect_resume")
{
	...
	$wheresql=" WHERE a.company_uid='{$_SESSION['uid']}' ";
	$row=$db->getall("select a.*,r.title,r.education_cn,r.birthdate,r.experience_cn,r.residence_cn from ".table("company_favorites")." as a left join ".table("resume")." as r on a.resume_id=r.id  $wheresql order by a.favoritesa_ddtime desc limit $offset,$rows");

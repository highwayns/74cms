...
elseif ($act == 'index')
{
	...
	$sql="select a.*,r.title,r.education_cn,r.birthdate,r.experience_cn,r.residence_cn from ".table("company_down_resume")." as a left join ".table("resume")." as r on a.resume_id=r.id  $wheresql order by a.down_addtime desc ".$limit;
	$row=$db->getall($sql);
...
elseif($act=="ajax_download_resume")
{
	...
	$wheresql=" WHERE a.company_uid='{$_SESSION['uid']}' ";
	$row=$db->getall("select a.*,r.title,r.education_cn,r.birthdate,r.experience_cn,r.residence_cn from ".table("company_down_resume")." as a left join ".table("resume")." as r on a.resume_id=r.id  $wheresql order by a.down_addtime desc limit $offset,$rows");
...
elseif($act=="ajax_download_resume_add")
{
	...
	else
	{
		...
		elseif($_CFG['operation_mode']=="1")
		{
			...
			$points=$resume['talent']=='2'?$points_rule['resume_download_advanced']['value']:$points_rule['resume_download']['value'];
...
elseif($act=="ajax_download_resume_add_save")
{
	...
	elseif($_CFG['operation_mode']=="1")
	{
				$points_rule=get_cache('points_rule');
				$points=$resume['talent']=='2'?$points_rule['resume_download_advanced']['value']:$points_rule['resume_download']['value'];
				$ptype=$resume['talent']=='2'?$points_rule['resume_download_advanced']['type']:$points_rule['resume_download']['type'];
				$mypoints=get_user_points($_SESSION['uid']);
				if  ($mypoints<$points)
				{
					exit("err");
